 ...
 background-size:100% 100%;">
    ...
    <section class="ftco-about img ftco-section ftco-no-pt ftco-no-pb" id="about-section" style="margin-top:150px;">
    	<div class="container shadow bg-white rounded-lg mb-4" data-aos="fade-up" data-aos-duration="2000" style="margin-top:150px;">
    		<div class="row ">
			<div class="col-lg-12">
			<h2 class="text-center mt-2">VIEW MARKS</h2>
...
<h2 class="text-center">Marks List</h2>
	<table class="table table-dark table-hover table-striped" id="tableData" border="2">
			<thead>
				<tr>
					<th>USN</th>
					<th>SUB CODE</th>
...
$sem='';
$sec='';
if(isset($_POST['submit']) && !empty($_POST['sem'])&& !empty($_POST['sec']) && !empty($_POST['subject'])){
	$sem=$_POST['sem'];
	$sec=$_POST['sec'];
	$sub=$_POST['subject'];
	$query="SELECT m.USN,m.sub_code,m.cie1,a.at_cie1,m.cie2,a.at_cie2,m.cie3,a.final_att,m.finalcie,m.assignment,m.activity,m.grade from marks m ,semsec ss,attendence a where m.usn=a.usn and m.ssid=ss.ssid and m.sub_code=a.sub_code and   ss.branch_id='$branch' and ss.sem='$sem' and ss.section='$sec' and m.sub_code='$sub'"; 
...
?>
  ...
  <td><?php echo $row['at_cie1'] ;?></td>
  <td><?php echo $row['cie2'] ;?></td>
  <td><?php echo $row['at_cie2'] ;?></td>
  <td><?php echo $row['cie3'] ;?></td>
  <td><?php echo $row['final_att'] ;?></td>
  <td><?php echo $row['assignment'] ;?></td>
  <td><?php echo $row['activity'] ;?></td>
  <td><?php echo $row['finalcie'] ;?></td>
