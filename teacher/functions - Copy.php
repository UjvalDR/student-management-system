...
?>
 ...
 function getSubjects($dbc,$Tid){
    $result = @mysqli_query($dbc, "select sub_code,sub_name,sem from subjects where teacher_id='$Tid'"); 
     echo '<select name="subject" class="custom-select">';
    if($result){
        while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
            if($row["sub_code"]==$subject){
                echo '<option value="'.$row["sub_code"].'" selected="selected">'.$row["sub_code"].' - '.$row["sub_name"].'</option>';
            }else{
...
function addStudentMarks($dbc,$query){
    $result = @mysqli_query ($dbc, $query); // Run the query.
    if ((mysqli_affected_rows($dbc) == 1) || (mysqli_affected_rows($dbc) == 0)) { 
        echo '<script>
alert("Marks Sucessfully Added");
</script>	'	;					
    } else { // If query did not run OK.
        echo '<script>alert("OOPS..!!! Please try again");</script>';		// Public message.
...
function addAttendence($dbc,$query){
    $result = @mysqli_query ($dbc, $query); // Run the query.
    if ((mysqli_affected_rows($dbc) == 1) || (mysqli_affected_rows($dbc) == 0)) { 
        echo '<script>
alert("Attendence Sucessfully Added");
</script>	'	;					
    } else { // If query did not run OK.
        echo '<script>alert("OOPS..!!! Please try again");</script>';		// Public message.
	    echo '<p>' . mysqli_error($dbc) . '<br /><br />Query: ' . $query . '</p>';
